...
public class FortuneServlet extends HttpServlet{
	...
	protected void service(HttpServletRequest req, HttpServletResponse resp) throws ServletException, IOException {
		//오늘의 운세를 얻어오는 비즈니스 로직을 수행(DB에서 읽어왔다가 가정)
		String[] fortuneList={
				"동쪽으로 가면 귀인을 만나요!",
				"오늘은 운전하지 마세요.",
				"오늘은 집에만 있는게 좋아요.",
				"오늘은 친구와 다툴수도 있어요.",
				"오늘은 운명의 상대를 만날지도 몰라요.",
				"오늘은 금전운이 좋은 날이에요",
				"가급적 과음은 피하세요"
				};
		int num = new Random().nextInt(fortuneList.length);
		String fortune = fortuneList[num];
		 ...
		 * 단, 담을 때는 Object type으로 받아주기 때문에 어떤 type이든 담을 수 있지만
		...
		/*
		 * webapp/test/fortune.jsp 페이지에 응답을 대신하도록 하고
		 * 서블릿에 전달받았던 HttpServletRequest, HttpServletResponse 객체의 참조값을 전달해준다.
		 */
		rd.forward(req, resp);
	}
}
